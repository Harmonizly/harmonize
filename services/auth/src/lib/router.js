 ...
 * @param  {[type]} bagpipes [description]
 * @return {[type]}          [description]
 */
export function create(def: Object, bagpipes: Array<Object>): Function {
  const fittingDef = def;
  debug('config: %j', fittingDef);
  debug('bagpipes: %j', bagpipes);

  if (!controllers) {
    throw new Error('init must called before swagger is instantiated');
  }
  ...
  const controllers = fittingDef.controllers;
   ...
   * @return {[type]}       [description]
   */
  return function router(ctx: Object, cb: Function): void {
    const context = ctx;
    debug('exec');
